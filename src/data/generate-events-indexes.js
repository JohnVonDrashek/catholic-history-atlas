...
const centuryDirs = fs.readdirSync(eventsDir)
  .filter(item => {
...
centuryDirs.forEach(centuryDir => {
  const centuryPath = path.join(eventsDir, centuryDir);
  const eventFiles = fs.readdirSync(centuryPath)
    .filter(file => file.endsWith('.json'))
    .sort();
  
  if (eventFiles.length === 0) {
    console.warn(`No event files found in ${centuryDir}`);
    return;
  }
  ...
  const createVarName = (id) => {
    ...
    // If it starts with a number, prefix with 'event_'
    if (/^\d/.test(varName)) {
      varName = 'event_' + varName;
    }
    return varName;
  };
  
  // Generate import statements
  const imports = eventFiles.map(file => {
    const eventId = file.replace('.json', '');
    const varName = createVarName(eventId);
    return `import ${varName} from './${file}';`;
  }).join('\n');
  
  // Generate export array
  const exports = eventFiles.map(file => {
    const eventId = file.replace('.json', '');
    const varName = createVarName(eventId);
    return `  ${varName}`;
  }).join(',\n');
...
`;
  ...
  console.log(`Generated ${indexPath} with ${eventFiles.length} events`);
});
